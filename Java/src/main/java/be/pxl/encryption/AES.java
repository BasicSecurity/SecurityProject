...
public class AES {
	...
	static String message = "test text 123\0\0"; /* Note null padding */
	static String messageNoPadding = "test text 123 sqmldfkjsdqmlfk";
	static String encryptionKey1 = "0123456789abcdef"; 					//128 bits
	static String encryptionKey2 = "0123456789abcdefghijklmn";			//192 bits
	static String encryptionKey3 = "0123456789abcdef0123456789abcdef";	//256 bits
	static String salt = "imma salt!";
	...
	public static void main(String[] args) {
		// Set configuration
		configuration.setAlgorithm(Algorithm.PBKDF2WithHmacSHA512);
		configuration.setPassword("0123456789abcdef0123456789abcdef");
		configuration.setSalt("imma salt!");
		configuration.setKeySize(256);
		configuration.setIterations(65536);
		configuration.setIv("AAAAAAAAAAAAAAAA");
		
		try {
//			byte[] encryptedMessage = encryptAes(messageNoPadding, encryptionKey3, salt, Algorithm.PBKDF2WithHmacSHA512); // Implementation.AES_CBC_PKCS5PADDING
//			String decryptedMessage = decryptAes(encryptedMessage, encryptionKey3, iv, Algorithm.PBKDF2WithHmacSHA512);
			
			byte[] encryptedMessage = encryptAes(message, configuration);
			String decryptedMessage = decryptAes(encryptedMessage, configuration);
			
			System.out.printf("%-16s%s\n", "Plain text:", messageNoPadding);
			System.out.println(printEncrypted(encryptedMessage));
			System.out.println(printDecrypted(decryptedMessage));
	...
	// encrypt
	public static byte[] encryptAes(String message, Configuration configuration) 
	throws NoSuchAlgorithmException, InvalidKeySpecException, InvalidKeyException, 
			InvalidAlgorithmParameterException, UnsupportedEncodingException, IllegalBlockSizeException, 
			BadPaddingException, NoSuchPaddingException, InvalidParameterSpecException
	{
			//SecretKeyFactory factory = SecretKeyFactory.getInstance("PBKDF2WithHmacSHA1");	// 160 bits hashlength
			SecretKeyFactory factory = SecretKeyFactory.getInstance(configuration.getAlgorithm().get());	// 512 bits hashlength
			KeySpec spec = new PBEKeySpec(
					configuration.getPassword().toCharArray(), 
					configuration.getSalt().getBytes(), 
	...
	// decrypt
	public static String decryptAes(byte[] encrypted, Configuration configuration) 
			throws NoSuchAlgorithmException, InvalidKeySpecException, InvalidKeyException, 
			InvalidAlgorithmParameterException, UnsupportedEncodingException, IllegalBlockSizeException, 
	...
	private static String printEncrypted(byte[] cipher){
		return String.format("%-16s%s","Encrypted: ", getString(cipher));
	}
	
	private static String printDecrypted(String message){
		return String.format("%-16s%s","Decrypted: ", message);
	}
	
	private static String getString(byte[] cipher){
...
//	public static byte[] encrypt(String message, String encryptionKey) throws Exception {
...
//	}
//	
//	public static byte[] encrypt2(String message, String encryptionKey, Algorithm algorithm) throws Exception {
...
//	}
//	
//	public static byte[] encryptAes256(String message, String IV, String encryptionKey){
//		try {
...
//	private static int[] getInts(byte[] cipher) {
