...
public class CopyOfAes {
	...
	public SecretKey createSecretKey(KeySettings keySettings)
		...
		if (keySettings.getAlgorithm().equals("PBKDF2WithHmacSHA1")
			...
			SecretKeyFactory factory = SecretKeyFactory.getInstance(keySettings
					.getAlgorithm()); // // AES/CBC/PKCS5Padding. Options see
	...
	public static void main(String[] args) {
		...
		String text = "abcdefghijklmnop"; // != 16 bytes will fail with
											// nopadding
		...
		// BEGIN
		try {
			byte[] encryptedMessage = aes.encrypt(Serializer.serialize(text), // serialize
																				// =
																				// omzetten
																				// naar
																				// byte
